...
INSERT INTO node_runs(
    ...
    graph_revision, generation, state, remaining_dependencies, created_at, updated_at
)
SELECT ?, wr.id, wr.definition_id, wr.definition_version, ?, ?, ?, ?, ?, ?, ?
FROM workflow_runs wr
WHERE wr.id=?`, string(nr.ID), string(nr.NodeID), nr.GraphRevision, nr.Generation, string(nr.State), nr.RemainingDependencies, formatTime(nr.CreatedAt), formatTime(nr.UpdatedAt), string(nr.WorkflowRunID))
	if err != nil {
