...
func BenchmarkIsolatedWriteRouterDispatch100Operations(b *testing.B) {
	ctx := context.Background()
	db, err := harnesssqlite.Open(ctx, filepath.Join(b.TempDir(), "bench_write_router.db"), harnesssqlite.Options{})
	if err != nil {
		b.Fatal(err)
	}
	defer db.Close()
	...
	for i := 0; i < b.N; i++ {
		for j := 0; j < 100; j++ {
			...
			env.ID = harnessmodel.TaskEnvelopeID(fmt.Sprintf("tenv_bench_write_%d_%d", i, j))
			env.AttemptID = harnessmodel.AttemptID(fmt.Sprintf("att_bench_write_%d_%d", i, j))
			env.Workspace.IsolationRoot = fmt.Sprintf("c:/repo/.scratch/bench_%d_%d", i, j)

			route, err := router.RouteIsolatedWrite(ctx, env, planText)
			if err != nil || route.Assignment.ID == "" {
				b.Fatalf("RouteIsolatedWrite failed: %v", err)
			}
		}
	}
}
