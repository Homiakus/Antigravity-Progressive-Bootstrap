...
var (
	// ErrReadOnlyRequired indicates an attempt to route an envelope that allows writes to read-only route.
	ErrReadOnlyRequired = errors.New("read-only routing violation: envelope workspace is not marked read-only")
	// ErrIsolatedWriteRequired indicates an attempt to route a write envelope without write isolation.
	ErrIsolatedWriteRequired = errors.New("isolated write routing violation: envelope workspace is not marked isolated with isolation root")
	...
	// ErrExecutionFailed indicates execution returned an error.
...
// IsolatedWriteOutput records the result and modified files of an isolated write execution.
type IsolatedWriteOutput struct {
...
// IsolatedWriteExecutorFunc defines the execution callback for a routed isolated write task.
// Implementations MUST perform mutations strictly inside the designated IsolationRoot or WorktreeID.
type IsolatedWriteExecutorFunc func(ctx context.Context, env harnessmodel.TaskEnvelope, assignment harnessmodel.ProviderAssignment) (output IsolatedWriteOutput, err error)
...
// RouteIsolatedWrite selects an optimal provider candidate and atomically commits the assignment
// and reservation in the SQLite store for a validated isolated write TaskEnvelope.
func (r *Router) RouteIsolatedWrite(ctx context.Context, env harnessmodel.TaskEnvelope, currentPlan []byte) (RouteResult, error) {
	if err := env.Validate(); err != nil {
		return RouteResult{}, fmt.Errorf("validate task envelope: %w", err)
	}

	// Invariant I-006 & I-032: Enforce write isolation constraints before dispatch.
	if env.Workspace.ReadOnly || !env.Workspace.Isolated || (strings.TrimSpace(env.Workspace.IsolationRoot) == "" && strings.TrimSpace(env.Workspace.WorktreeID) == "") {
		return RouteResult{}, fmt.Errorf("%w: task=%s rootPath=%s isolationRoot=%s worktreeId=%s",
			ErrIsolatedWriteRequired, env.TaskID, env.Workspace.RootPath, env.Workspace.IsolationRoot, env.Workspace.WorktreeID)
	}
	...
	if err := r.store.Update(ctx, func(tx harnessstore.Tx) error {
		...
		if _, err := tx.GetAttempt(ctx, assignment.AttemptID); errors.Is(err, harnessstore.ErrNotFound) {
			...
			if err := tx.CreateWorkflowDefinition(ctx, harnessmodel.WorkflowDefinition{
				ID: defID, Version: 1, Name: "standalone-write", CreatedAt: now,
				Nodes: []harnessmodel.NodeSpec{{ID: nodeID, Kind: harnessmodel.NodeKindAction, ExecutorKind: harnessmodel.ExecutorAgent}},
			}); err != nil {
				return fmt.Errorf("create standalone workflow def: %w", err)
			}
			...
			if err := tx.CreateGraphRevision(ctx, harnessmodel.GraphRevision{
				WorkflowRunID: wfrID, Number: 1, CreatedAt: now, Reason: "standalone-write-route",
			}); err != nil {
				return fmt.Errorf("create standalone graph revision: %w", err)
			}
			if err := tx.CreateNodeRun(ctx, harnessmodel.NodeRun{
...
// ExecuteIsolatedWrite safely runs an isolated write executor against a verified route result,
// updating the durable ledger upon completion.
func (r *Router) ExecuteIsolatedWrite(ctx context.Context, route RouteResult, exec IsolatedWriteExecutorFunc) (ExecutionResult, error) {
	if route.Assignment.ID == "" || route.Assignment.State != harnessmodel.ProviderAssignmentActive {
		return ExecutionResult{Route: route}, ErrInvalidRouteState
	}
	if route.Envelope.Workspace.ReadOnly || !route.Envelope.Workspace.Isolated {
		return ExecutionResult{Route: route}, ErrIsolatedWriteRequired
	}

	startTime := r.opts.Now()
	out, execErr := exec(ctx, route.Envelope, route.Assignment)
	duration := r.opts.Now().Sub(startTime)
	now := r.opts.Now().UTC()
	...
	updateErr := r.store.Update(ctx, func(tx harnessstore.Tx) error {
		...
		if out.TokensUsed > 0 && route.Reservation != nil {
			sample := harnessmodel.ProviderUsageSample{
				Key:           fmt.Sprintf("usamp_%d_%s", now.UnixNano(), route.Assignment.ID),
				AssignmentID:  route.Assignment.ID,
				ReservationID: route.Reservation.ID,
