...
1. **Левая колонка (`◈ NAVIGATION`)**: боковое меню с быстрыми метками разделов (`01`..`05`), иконками и мгновенным переходом по цифрам `1`..`5`.
2. **Центральная колонка (`WORKSPACE`)**: интерактивная рабочая зона (метрики, переключатели режимов, таблицы MCP, списки задач, параметры безопасности).
3. **Правая колонка (`> LIVE CONSOLE & LOGS`)**: встроенный живой терминал логов с подсветкой статусов (`[OK]`, `[CMD]`, `[INFO]`, `[WARN]`, `[ERROR]`) и возможностью очистки (`c`).
4. **Keyboard-First управление**:
   - `Tab` / `Shift+Tab` или `←` / `→` — удобный переход фокуса между колонками.
