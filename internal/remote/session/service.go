...
type Options struct {
	...
	WorkspaceAllocator remoteworkspace.Allocator
...
func New(opts Options) (*Service, error) {
	...
	return &Service{
		...
		workspaceAllocator: opts.WorkspaceAllocator,
		ids:                ids,
		secrets:            secrets,
		hostID:             opts.HostID,
...
func (s *Service) Provision(ctx context.Context, spec Spec) (model.RemoteSession, error) {
	...
	if !spec.IsolationMode.Valid() {
		return model.RemoteSession{}, fmt.Errorf("invalid isolation mode %q", spec.IsolationMode)
	}
	if spec.IsolationMode == model.IsolationWorktree && s.workspaceAllocator == nil {
		return model.RemoteSession{}, fmt.Errorf("WORKTREE isolation requires Harness workspace allocator")
	}
	...
	sessionIDValue, err := s.newID(model.IDRemoteSession)
	if err != nil {
		return model.RemoteSession{}, err
	}
	workspaceIDValue, err := s.newID(model.IDWorkspace)
	if err != nil {
		return model.RemoteSession{}, err
	}
	conversationIDValue, err := s.newID(model.IDConversation)
	if err != nil {
		return model.RemoteSession{}, err
	}
	sessionID := model.RemoteSessionID(sessionIDValue)
	workspaceID := model.WorkspaceID(workspaceIDValue)

	allocation := remoteworkspace.Allocation{
		ID:       workspaceID,
		Path:     repo.CanonicalPath,
		Mode:     spec.IsolationMode,
		RepoRoot: repo.GitRoot,
	}
	workspaceAllocated := false
	workspaceCommitted := false
	if s.workspaceAllocator != nil {
		allocation, err = s.workspaceAllocator.Allocate(ctx, remoteworkspace.Request{
		...
		if err != nil {
			return model.RemoteSession{}, fmt.Errorf("allocate remote workspace: %w", err)
		}
	...
	if workspaceAllocated {
		defer func() {
			...
			defer cancel()
			_ = s.workspaceAllocator.Release(releaseCtx, allocation)
		}()
	}
	workspacePath := allocation.Path

	instance, _, located, err := s.resolveInstance(ctx, spec, repo, sessionID, workspacePath)
	if err != nil {
		return model.RemoteSession{}, err
	}
	wasRunning := instance.Running
	startedByUs := false
	rollbackInstance := func() {
	...
	if !containsPath(ideContext.WorkspaceFolders, workspacePath) {
		rollbackInstance()
		return model.RemoteSession{}, fmt.Errorf("expected %s in IDE workspace: %w", workspacePath, ErrWorkspaceMismatch)
	}
	...
	conversation := model.Conversation{
		ID:                     model.ConversationID(conversationIDValue),
...
func (s *Service) resolveInstance(ctx context.Context, spec Spec, repo model.Repository, sessionID model.RemoteSessionID, workspacePath string) (cockpit.Instance, bool, *antigravityide.LocatedBridge, error) {
	...
	case InstanceAuto:
		...
		if decision.Action == ResolveReuse {
			...
			if !samePath(instance.WorkingDir, workspacePath) {
				if instance.Running {
					return s.createDedicatedInstance(ctx, spec, repo, sessionID, workspacePath)
				}
				patched := workspacePath
				instance, err = s.cockpit.UpdateInstance(ctx, instance.ID, cockpit.InstancePatch{WorkingDir: &patched})
				if err != nil {
					return cockpit.Instance{}, false, nil, fmt.Errorf("replace Cockpit workspace: %w", err)
				}
			}
			if instance.Running {
	...
	case InstanceExisting:
		...
		if !samePath(instance.WorkingDir, workspacePath) {
			return cockpit.Instance{}, false, nil, fmt.Errorf("Cockpit instance working_dir %q does not match requested workspace %q: %w", instance.WorkingDir, workspacePath, ErrWorkspaceMismatch)
		}
		if instance.Running {
			if located, ok := s.liveBridge(instance.ID); ok {
				return instance, false, &located, nil
...
func (s *Service) createDedicatedInstance(ctx context.Context, spec Spec, repo model.Repository, sessionID model.RemoteSessionID, workspacePath string) (cockpit.Instance, bool, *antigravityide.LocatedBridge, error) {
