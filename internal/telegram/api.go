...
func NewAPIClient(token string, opts APIOptions)(*APIClient,error){
	token=strings.TrimSpace(token);if token==""{return nil,fmt.Errorf("telegram bot token is required")}
	baseURL:=strings.TrimSpace(opts.BaseURL);if baseURL==""{baseURL="https://api.telegram.org"};base,err:=url.Parse(baseURL);if err!=nil{return nil,fmt.Errorf("parse Telegram API base URL: %w",err)}
	if base.Scheme!="https" && !(base.Scheme=="http"&&isLoopback(base.Hostname())){return nil,fmt.Errorf("Telegram API requires HTTPS except loopback tests")}
	client:=opts.HTTPClient;if client==nil{client=&http.Client{Timeout:45*time.Second}}
	return &APIClient{base:base,token:token,http:client},nil
}
...
func (c *APIClient) GetUpdates(ctx context.Context,offset int64,timeout time.Duration)([]Update,error){
	seconds:=int(timeout/time.Second);if seconds<1{seconds=1};if seconds>50{seconds=50}
	var out []Update
	err:=c.call(ctx,"getUpdates",map[string]any{"offset":offset,"timeout":seconds,"limit":100,"allowed_updates":[]string{"message","callback_query"}},&out)
	return out,err
}

func (c *APIClient) SendMessage(ctx context.Context,chatID,threadID int64,text string)error{
	if chatID==0||strings.TrimSpace(text)==""{return fmt.Errorf("telegram chat and text are required")};body:=map[string]any{"chat_id":chatID,"text":text};if threadID!=0{body["message_thread_id"]=threadID};var ignored Message;return c.call(ctx,"sendMessage",body,&ignored)
}
