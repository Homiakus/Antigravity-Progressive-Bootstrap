...
func (s *Store) GetTelegramPrincipal(ctx context.Context, userID int64) (model.TelegramPrincipal, error) {
	...
	var enabled int
	var paired string
	err := s.db.QueryRowContext(ctx, `SELECT user_id,role,enabled,paired_at FROM telegram_principals WHERE user_id=?`, userID).Scan(&item.UserID, &item.Role, &enabled, &paired)
	if errors.Is(err, sql.ErrNoRows) { return model.TelegramPrincipal{}, remotestore.ErrNotFound }
	if err != nil { return model.TelegramPrincipal{}, err }
...
func (s *Store) CreateTelegramPairing(ctx context.Context, pairing model.TelegramPairing) error {
	...
	_, err := s.db.ExecContext(ctx, `INSERT INTO telegram_pairings(token_hash,role,intended_chat_id,created_at,expires_at,consumed_at,consumed_by_user_id) VALUES(?,?,?,?,?,?,?)`, pairing.TokenHash, pairing.Role, pairing.IntendedChatID, formatTime(pairing.CreatedAt), formatTime(pairing.ExpiresAt), nil, nil)
...
func (s *Store) ConsumeTelegramPairing(ctx context.Context, tokenHash string, userID, chatID int64, now time.Time) (model.TelegramPrincipal, error) {
	if strings.TrimSpace(tokenHash) == "" || userID == 0 || now.IsZero() { return model.TelegramPrincipal{}, fmt.Errorf("telegram pairing hash, user and time are required") }
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil { return model.TelegramPrincipal{}, err }
	defer tx.Rollback()
	var role model.TelegramRole
	var intended int64
	var expires string
	var consumed sql.NullString
	err = tx.QueryRowContext(ctx, `SELECT role,intended_chat_id,expires_at,consumed_at FROM telegram_pairings WHERE token_hash=?`, tokenHash).Scan(&role, &intended, &expires, &consumed)
	if errors.Is(err, sql.ErrNoRows) { return model.TelegramPrincipal{}, remotestore.ErrNotFound }
	if err != nil { return model.TelegramPrincipal{}, err }
	if consumed.Valid { return model.TelegramPrincipal{}, remotestore.ErrConflict }
	expiresAt, err := parseTime(expires)
	if err != nil { return model.TelegramPrincipal{}, err }
	if !now.Before(expiresAt) { return model.TelegramPrincipal{}, fmt.Errorf("telegram pairing expired: %w", remotestore.ErrConflict) }
	...
	result, err := tx.ExecContext(ctx, `UPDATE telegram_pairings SET consumed_at=?,consumed_by_user_id=? WHERE token_hash=? AND consumed_at IS NULL`, formatTime(now), userID, tokenHash)
	if err != nil { return model.TelegramPrincipal{}, err }
	rows, err := result.RowsAffected(); if err != nil { return model.TelegramPrincipal{}, err }; if rows != 1 { return model.TelegramPrincipal{}, remotestore.ErrConflict }
	...
	if err := tx.Commit(); err != nil { return model.TelegramPrincipal{}, err }
...
func (s *Store) ReserveTelegramCallback(ctx context.Context, callbackID string, userID, chatID int64, receivedAt time.Time) (bool, error) {
	if strings.TrimSpace(callbackID)=="" || userID==0 || receivedAt.IsZero() { return false, fmt.Errorf("telegram callback id, user and time are required") }
	_, err := s.db.ExecContext(ctx, `INSERT INTO telegram_callback_replay(callback_query_id,user_id,chat_id,received_at) VALUES(?,?,?,?)`, callbackID, userID, chatID, formatTime(receivedAt))
	if err == nil { return true, nil }
	var count int
	if qerr := s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM telegram_callback_replay WHERE callback_query_id=?`, callbackID).Scan(&count); qerr == nil && count == 1 { return false, nil }
	return false, err
}
...
func (s *Store) UpsertTelegramBinding(ctx context.Context, binding model.TelegramBinding) error {
	...
	if err != nil { return mapWriteError("upsert telegram binding", err) }
	return nil
}
...
func (s *Store) GetTelegramBindingByTopic(ctx context.Context, chatID, threadID int64) (model.TelegramBinding, error) {
	...
	var enabled int
	var created string
	err := s.db.QueryRowContext(ctx, `SELECT id,session_id,chat_id,thread_id,owner_user_id,enabled,created_at FROM telegram_bindings WHERE chat_id=? AND thread_id=? AND enabled=1`, chatID,threadID).Scan(&item.ID,&item.SessionID,&item.ChatID,&item.ThreadID,&item.OwnerUserID,&enabled,&created)
